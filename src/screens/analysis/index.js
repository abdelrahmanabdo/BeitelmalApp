...
const Analysis = () => {
  ...
  const fileEndpointUrl = 'http://beitelmal.info/public/uploads-files/';

  const getData = async (page = 1) => {
    await api
      .get(`${endpoints.analysis}?page=${page}`)
      .then((res) => {
        setLastPage(res.data.UPLOADS.last_page);
        let newData = data;
        newData.push(...res.data.UPLOADS);
        setData(newData);
        setIsLoading(false);
      })
          ...
          data.length > 0 ?
            <FlatList
              ...
              renderItem={({item}) => <View key={item.id} style={style.card}>
                <FastImage 
                  source={item.file ? {uri: `${fileEndpointUrl}${item.file}`} : require('../../assets/images/default-image.png')}
                  style={style.image}
                  resizeMode="stretch"
                />
